...
public class Enemigo {
    ...
    public void actualizaCoordenadas(){
        if(tipo_enemigo==ENEMIGO_INTELIGENTE) {
            if (juego.nave.getX() > coordenada_x)
                coordenada_x+=velocidad;
            else if (juego.nave.getX() < coordenada_x)
                coordenada_x-=velocidad;
            if(Math.abs(coordenada_x-juego.nave.getX())<velocidad)
                coordenada_x=juego.nave.getX(); //if it's close enough it takes the same position
